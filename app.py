...
def home():
    """
    Renders Home library page if user logged in with contents shown in date order fo creation.
    If user is not logged in, they are rendered to the app landing page to register or login
    """
...
def score_up():
    """
    Increases the clicked articles up vote score by 1 when called by a logged in user.
...
def score_down(): 
    """
    Increases the clicked articles down vote score by 1 when called by a logged-in user.
    If not logged in, renders app landing page.
    """
    if session.get("user") is None:
        return render_template("index.html")
...
def search_fav(query):
    """
    Renders the library page with data related to the clicked favourite article.
    """
    if session.get("user") is None:
        return render_template("index.html")
...
def content(): 
    ...
    The data is taken from the form and stored in the Mongo DB content collection.
...
def editcontent(content_id):
    """
    Allows admin user to edit selected article and updates the revelent document in
...
def admin():
    """
    Renders admin page with details of all registered users, to an admin user only.
